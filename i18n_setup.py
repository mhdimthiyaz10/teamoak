...
# Pre-defined slug mapping for common UI strings
PREDEFINED_KEYS = {
    "home": "global_home",
...
def get_unique_key(text, page_prefix):
    ...
    # Prefix page text to keep keys namespace-grouped
...
def extract_and_annotate(element, string_to_key, page_prefix):
    ...
    # We target block text tags & inline tags if they are the primary containers
    if element.name in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'a', 'button', 'span', 'li', 'label', 'strong', 'em']:
        # It shouldn't contain nested blocks
        has_block_child = any(
        ...
        if not has_block_child:
            ...
            # Skip if it is small or purely formatting/numeric
...
def translate_batch(strings):
    ...
    translated = []
    # Translate in chunks of 30 to avoid rate limit or payload limits
    chunk_size = 30
    for i in range(0, len(strings), chunk_size):
        chunk = strings[i:i+chunk_size]
        print(f"Translating batch {i//chunk_size + 1} of {(len(strings)-1)//chunk_size + 1}...")
        try:
            results = translator.translate_batch(chunk)
            translated.extend(results)
        except Exception as e:
            print(f"Error in batch translation: {e}. Falling back to single translation.")
            for s in chunk:
                try:
                    translated.append(translator.translate(s))
                except Exception as ex:
                    print(f"Failed to translate string '{s}': {ex}")
                    translated.append(s) # keep original
    return translated

def cleanup_arabic_html(text):
    # Fix spacing added around tags by automatic translation
...
def main():
    ...
    print(f"HTML files annotated. Total unique strings: {len(string_to_key)}")
    
    # Translate strings
    english_strings = list(string_to_key.keys())
    print("Starting translation to Arabic...")
    arabic_strings = translate_batch(english_strings)
    ...
    # Standard manual professional overrides for key UI strings
    overrides = {
    ...
    print("en.json and ar.json successfully generated.")
